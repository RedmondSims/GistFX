...
public class DragFactory implements Callback<TreeView<DragNode>, TreeCell<DragNode>> {
    ...
    private TreeItem<DragNode>      dropParent;
    private TreeCell<DragNode>      styledCell;
    private int                     dropIndex;
    private TreeItem<DragNode>      draggedItem;


    public TreeCell<DragNode> call(final TreeView treeView) {
        TreeCell<DragNode> cell = new CellNode<>() {
            @Override
            public void updateItem(final DragNode item, final boolean empty) {
    ...
    private void dragDetected(final MouseEvent event, final TreeCell<DragNode> treeCell, final TreeView<DragNode> treeView) {
    ...
    private void dragOver(final DragEvent event, final TreeCell<DragNode> treeCell, final TreeView<DragNode> treeView) {
        ...
        if (y < (height * .5d)) {
            ...
            if (thisItem.getValue() instanceof GistType dragObj) {
    ...
    private int countParents(final TreeItem<DragNode> child) {
        int                ret     = 0;
        TreeItem<DragNode> current = child;
    ...
    private boolean childrenContains(final TreeItem<DragNode> targetItem, final TreeItem<DragNode> searchingFor) {
        boolean ret = false;
        for (TreeItem<DragNode> obj : targetItem.getChildren()) {
    ...
    private void drop(final DragEvent event, final TreeCell<DragNode> treeCell, final TreeView<DragNode> treeView) {
        if(draggedItem == null) return;
        boolean proceed     = false;
        boolean moveFile    = false;
        Type    draggedType = draggedItem.getValue().getType();
        Type    dropType    = dropParent.getValue().getType();
        ...
        if (draggedType.equals(GIST) && (dropType.equals(CATEGORY) || dropType.equals(STANDARD))) {
